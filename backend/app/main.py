...
# --- 包含功能模块的路由 ---
app.include_router(
    auth_router, prefix=settings.API_V1_STR, tags=["IAM - Authentication"]
)
app.include_router(
    user_router, prefix=f"{settings.API_V1_STR}/users", tags=["IAM - Users"]
)
app.include_router(
    admin_user_router, prefix=f"{settings.API_V1_STR}/admin/users", tags=["IAM - Admin"]
)

app.include_router(
    item_user_router, prefix=f"{settings.API_V1_STR}/items", tags=["Items"]
)
app.include_router(
    item_admin_router,
    prefix=f"{settings.API_V1_STR}/admin/items",
    tags=["Admin - Items"],
)
# --- 添加全局中间件 ---
