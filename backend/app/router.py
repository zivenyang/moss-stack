        ...
        try:
            ...
            if hasattr(module, "router"):
                ...
                if isinstance(router_obj, APIRouter):
                    ...
                    print(
                        f"  ✅ SUCCESS: Auto-discovered and included router from: {module_info.name}"
                    )
                else:
                    print(
                        f"  ⚠️ WARNING: Found 'router' in {module_info.name}, but it is not an APIRouter instance."
                    )
            else:
                print(
                    f"  ℹ️ INFO: Module {module_info.name} matches pattern but has no 'router' attribute."
                )
